...
class grpcConan(ConanFile):
    ...
    def validate(self):
        if is_msvc(self):
            if self.settings.compiler == "Visual Studio":
                ...
                vs_ide_version = visual.msvc_version_to_vs_ide_version(
        ...
        if self.options.shared and (not self.options["protobuf"].shared or not self.options["googleapis"].shared or not self.options["grpc-proto"].shared):
            raise ConanInvalidConfiguration(
                "If built as shared, protobuf, googleapis and grpc-proto must be shared as well. Please, use `protobuf:shared=True` and `googleapis:shared=True` and `grpc-proto:shared=True`")
    ...
    def _patch_sources(self):
        ...
        shutil.rmtree(os.path.join(
            self._source_subfolder, "src", "proto", "grpc"))
    ...
    def _create_executable_module_file(self, target, executable):
        ...
        replace_in_file(
            self, dst_file, "@find_program_variable@", find_program_var)

        module_folder_depth = len(os.path.normpath(
            self._module_path).split(os.path.sep))
    ...
    def package_info(self):
        ...
        for component, values in self._grpc_components.items():
            ...
            self.cpp_info.components[component].set_property(
                "pkg_config_name", target)
            self.cpp_info.components[component].libs = [lib]
            self.cpp_info.components[component].requires = values.get(
                "requires", [])
        ...
        for plugin_option, values in self._grpc_plugins.items():
            if self.options.get_safe(plugin_option):
                ...
                grpc_modules.append(os.path.join(
                    self._module_path, grpc_module_filename))
